...
def generate_pdf(data_dict, svg_string=None):
    try:
        buffer = BytesIO()
        ...
        try:
            ...
            c.setFillColor("red")
            c.drawString(40, height - 60, "Image non disponible")
            c.setFillColor("black")

        # 2. EN-TÊTE (Client, Ref, Prix)
        c.setFont("Helvetica-Bold", 16)
        c.drawString(40, height - 100, f"Devis / Fiche Technique")
        
        c.setFont("Helvetica", 10)
        c.drawString(40, height - 120, f"Client : {data_dict.get('project', {}).get('name', 'Client Anonyme')}")
        c.drawString(40, height - 135, f"Réf : {data_dict.get('ref_id', 'F1')}")
        # Prix (Placeholder si non dispo)
        c.drawString(40, height - 150, f"Prix : {data_dict.get('price', 'Sur Demande')}")
        
        c.drawString(40, height - 165, f"Date : {datetime.datetime.now().strftime('%d/%m/%Y')}")

        # 3. DETAILS
        y_cursor = height - 190
        c.setFont("Helvetica-Bold", 12)
        c.drawString(40, y_cursor, "Caractéristiques")
        y_cursor -= 20
        c.setFont("Helvetica", 10)
        
        infos = [
            f"Dim: {data_dict.get('width_dorm', 0)} x {data_dict.get('height_dorm', 0)} mm",
            f"Matériau: {data_dict.get('mat_type', 'PVC')}",
            f"Couleur: {data_dict.get('col_in','-')} / {data_dict.get('col_ex','-')}",
        ...
        # 4. SCHÉMA
        c.setFont("Helvetica-Bold", 12)
        c.drawString(40, y_cursor, "Plan Technique")
        y_cursor -= 20
        ...
        if svg_string:
            try:
                ...
                c.setFillColor("red")
                c.drawString(40, y_cursor, "Image non disponible")
                c.setFillColor("black")
                y_cursor -= 40
        else:
            c.drawString(40, y_cursor, "Image non disponible")
...
# Use the function
        pdf_buffer = generate_pdf(s, svg_output)
